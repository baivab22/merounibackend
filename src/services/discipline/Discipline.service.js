...
class DisciplineService {
  ...
  async getDisciplineBySlug(slug) {
      ...
      where: {
        [Op.and]: [
          Sequelize.where(
            Sequelize.fn(
              "JSON_CONTAINS",
              Sequelize.col("disciplines"),
              JSON.stringify(Number(discipline.id)),
            ),
            true,
          ),
          { status: "published" },
  ...
  async createDiscipline(payload) {
    const { title, slug: customSlug, ...rest } = payload;
    const slug = customSlug || generateUniqueSlug(title);
